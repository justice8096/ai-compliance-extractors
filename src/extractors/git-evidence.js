 ...
 * Rejects paths containing null bytes or non-absolute paths, which could
 * cause argument injection in git's argument parsing (CWE-88 mitigation).
 *
...
/**
 * Run a git command in the given repository using execFileSync.
 *
 * Security: Uses execFileSync with arguments array instead of shell string.
 * The repoPath is passed via -C flag and not interpolated into a shell string.
 * All subcommands are hardcoded, no user input is injected.
 */
